...
$qc=mysql_query("select * from comment where status='D' order by tgl desc");
$qp=mysql_query("select * from pesan where status='D' order by tgl desc");
$jmlp=mysql_num_rows($qp);
$jmlc=mysql_num_rows($qc);
//$dkomen=mysql_fetch_assoc($qc);
//$dpesan=mysql_fetch_assoc($qp);
error_reporting();
?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <title>Administrator | CENTRAL LAUNDRY</title>
...
<script language="javascript">
        ...
        function EditdataUser(){
            ...
            if (EditU.adminpsw1.value !=EditU.adminpsw2.value){
            alert("Kedua password harus sama");
            EditU.adminpsw1.focus();
            return false
        }    
            if(! nama.EditU(EditU.adminnama.value)){
            alert('Hanya isikan huruf pada nama');
            EditU.adminnama.focus();
            return false
        }
        return true
    }
        ...
        function ValAddUser(){
            if(! nama.adduser(adduser.adminnama.value)){
            alert('Hanya isikan huruf pada nama');
            adduser.adminnama.focus();
            return false
        }
            if (adduser.adminpsw1.value.length<3)
        {
            alert("Password minimal 3 karakter");
            adduser.adminpsw1.focus()
            return false
        }
        return true
    }
        function ValEditUser(){
            if(! nama.editusr(editusr.adminnama.value)){
            alert('Hanya isikan huruf pada nama');
            editusr.adminnama.focus();
            return false
        }
            if (editusr.adminpsw1.value.length<3){
            alert("Password minimal 3 karakter");
            editusr.adminpsw1.focus()
            return false
        }
            return true
    }
                                    ...
                                    <?php while($dc=mysql_fetch_assoc($qc)){ ?>
                                        <li><!-- start message -->
                                            <a href="media.php?fModule=detailcomment&id=<?php echo $dc['no'] ?>">
                                                ...
                                                <h4>
                                                    <?php echo ucwords($dc['nama']); ?><small><i class="fa fa-calendar"> <?php echo tgl_indo($dc['tgl']) ?></i></small>
                                                </h4>
                                                <p><?php echo substr($dc['pesan'],0,22) ?></p>
                                            </a>
                                        </li><!-- end message -->
                                    <?php } ?>
                                    </ul>
                        ...
                        <li class="dropdown user user-menu">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li class="user-footer">
                                    <div class="pull-left">
                                    <?php
                                        $qu=mysql_query("select * from user where kd_user='{$_SESSION['kd_user']}'");
                                        $du=mysql_fetch_array($qu);
                                    ?>
                                        <a href="#" onclick="javascript:editadmin('<?php echo $du['kd_user'] ?>','<?php echo $du['nama'] ?>','<?php echo $du['username'] ?>')" class="btn btn-default btn-flat">Profile</a>
                                    </div>
...
<div class="modal fade" id="ModalEditUserAktif" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
                        <form  name="EditU" method="post" action="pages/dashboard/aksidashboard.php?aksi=edituser" onSubmit="return EditdataUser()">                      
                            ...
                            <label>Nama</label>
                                <input class="form-control input-sm" required type="text" name="adminnama" placeholder="Nama Lengkap"/>
                            <hr>
                            <label>Username</label>
                                <input class="form-control input-sm" required type="text" name="adminusr" placeholder="Username"/>
                            <hr>                  
                            <label>Password Baru</label>
                                <input class="form-control input-sm" required type="password" name="adminpsw1" placeholder="Password"/>
                            <hr>
                            <label>Ulangi Password</label>
                                <input class="form-control input-sm" required type="password" name="adminpsw2" placeholder="Ulangi Password"/>
            </div>
    ...
    function editadmin(no,nama,username){
       ...
       //var userpwd=psw;
        ...
        document.EditU.adminnama.value=namauser;
        document.EditU.adminusr.value=penggunauser;
